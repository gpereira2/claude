...
where the failure was first observed. The `harness-sync` routine's VERIFY step
named both write-race traps explicitly; its RECORD step, one paragraph later,
prescribed a bare stamp write. On 2026-08-19 the documented trap fired exactly
there: an unawaited write raced its same-script read-back and
`last-plugin-sync.txt` kept the previous run's date. That is not cosmetic — the
next run reads that stamp to compute its window, so staleness widens the window
and risks re-porting what the plugin already carries.

*Seen once more on 2026-08-20,* in the run that ported this corollary: the
replacement stamp write was **rejected** by the write-gate, and the run learned
this only because the result was captured rather than discarded. A run that
collapsed it to `ok` would have reported a stamp it never wrote.
*(From Observation 43, verified 2026-08-20.)*
